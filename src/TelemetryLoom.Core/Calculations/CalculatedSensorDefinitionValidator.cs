...
public sealed partial class CalculatedSensorDefinitionValidator(SensorAliasRegistry aliases)
{
    ...
    public CalculatedDefinitionValidationResult Validate(
        ...
        string displayName,
        string formula,
        IReadOnlyCollection<CalculatedSensorDefinition> existingDefinitions)
    {
        ValidateKey(key);
        var name = ValidateDisplayName(displayName);
        var normalizedFormula = ValidateFormulaText(formula);
        if (aliases.GetDefinitions().Any(alias => string.Equals(alias.Key, key, StringComparison.Ordinal)))
        {
        ...
        try
        {
            expression = FormulaEngine.Parse(normalizedFormula);
        }
        ...
        return new CalculatedDefinitionValidationResult(
            new CalculatedSensorDefinition(key, name, normalizedFormula, type.Quantity, type.Unit),
            FormulaEngine.GetDependencies(expression));
    }

    public IReadOnlyList<CalculatedSensorDefinition> ValidateLoaded(
        IReadOnlyList<CalculatedSensorDefinition> definitions)
    {
        try
        {
            ...
            foreach (var definition in definitions)
            {
                ...
                if (!Enum.IsDefined(definition.Quantity) || !Enum.IsDefined(definition.Unit))
                {
                    throw new CalculationValidationException(
                        $"Calculated sensor '{definition.Key}' has unknown unit metadata.");
                }
                ...
                {
                    throw new CalculationValidationException(
                        $"Calculated sensor '{definition.Key}' has inconsistent unit metadata.");
                }

                if (aliases.GetDefinitions().Any(alias =>
                        string.Equals(alias.Key, definition.Key, StringComparison.Ordinal)))
                {
                    throw new CalculationConflictException(
                        $"A physical sensor alias already uses key: {definition.Key}");
                }

                if (!byKey.TryAdd(definition.Key, definition))
                {
                    throw new CalculationConflictException(
                        $"Duplicate calculated sensor key: {definition.Key}");
                }
            }

            EnsureAcyclic(byKey);
            return [.. definitions.OrderBy(definition => definition.Key, StringComparer.Ordinal)];
        }
        catch (Exception exception) when (exception is FormulaException or CalculationValidationException or CalculationConflictException)
        {
            throw new InvalidDataException(
                $"Invalid calculated sensor configuration: {exception.Message}", exception);
        }
    }
        ...
        HashSet<string> visiting)
    {
        if (!visiting.Add(key))
        {
            throw new FormulaException($"Calculated sensor cycle includes '{key}'");
        }
        ...
        try
        {
            ...
            {
                var alias = aliases.GetDefinitions().FirstOrDefault(item => item.Key == dependency);
                if (alias is not null)
                {
                    return new FormulaType(alias.Quantity, alias.Unit);
                }
        ...
        IReadOnlyDictionary<string, CalculatedSensorDefinition> definitions)
    {
        ...
        void Visit(string key)
        {
            ...
            {
                throw new CalculationConflictException(
                    $"Calculated sensor cycle includes '{key}'.");
            }

            foreach (var dependency in FormulaEngine.GetDependencies(
                         FormulaEngine.Parse(definitions[key].Formula)))
            {
    ...
    private static void ValidateKey(string key)
    {
        ...
        {
            throw new CalculationValidationException(
                "Calculated sensor key must be 1-128 lowercase characters, start with a letter, and use alphanumeric segments separated by '.' or '_'.");
        }
    }
    ...
    private static string ValidateDisplayName(string displayName)
    {
        ...
        {
            throw new CalculationValidationException("Display name must be 1-256 characters.");
        }
    ...
    private static string ValidateFormulaText(string formula)
    {
        ...
        {
            throw new CalculationValidationException("Formula must be 1-2048 characters.");
        }
...
public sealed record CalculatedDefinitionValidationResult(
